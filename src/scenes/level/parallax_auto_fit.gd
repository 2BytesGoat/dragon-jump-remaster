...
## Automatically scales and tiles the first Sprite2D child so it covers the
## viewport plus the maximum parallax camera shift for this layer. Assumes
## scroll_scale already defines the desired parallax speed per axis.
...
func _fit(level_size: Vector2) -> void:
	...
	var parallax_shift := camera_range * Vector2(
		absf(1.0 - scroll_scale.x),
		absf(1.0 - scroll_scale.y)
	)
