...
- Loading level data via the `SceneLoader` autoload; if `GameSession.custom_level_code` is set, loads the level directly from the code string instead of a campaign resource
