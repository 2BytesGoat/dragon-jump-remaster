...
- **Main methods**:
  ...
  - `_on_settings_menu_close_me()`: Restores focusability via `_set_selection_focusable(true)`, hides the `SettingsMenu` overlay, and focuses the Play button (`focus_play_button()`)
  - `_set_selection_focusable(enabled)`: Makes the settings overlay focus-modal. Disabling walks `selection_container` and records every focusable control in `_focusable_controls` before setting its `focus_mode` to `FOCUS_NONE`; enabling restores `FOCUS_ALL` to exactly those recorded controls (and clears the list). Tracking the original set matters because a naive `FOCUS_ALL if enabled else FOCUS_NONE` restore is a no-op — after disabling, every control is `FOCUS_NONE`, so nothing would ever be restored
  - `focus_play_button()`: Grabs focus on the Play button (public so `main_screen.gd` can re-focus it when no control has focus)
  - `is_settings_open()` / `is_selection_visible()`: Public state queries used by `main_screen.gd`'s up/down re-focus fallback
...
- **Main methods**:
  ...
  - `_unhandled_input()`: When the main menu's selection container is visible, settings is closed, and no control has focus (e.g. the cursor left a button), `ui_up` / `ui_down` re-grab focus on the Play button via `main_menu.focus_play_button()` — the same fallback the settings menu has for its own controls
...
- **Settings menu: focus follows the cursor.** `settings.gd` `_setup_hover_focus()` wires `mouse_entered` → `grab_focus()` and `mouse_exited` → `release_focus()` on the three sliders and the CRT checkbox, so the focus ring tracks the cursor and disappears when it leaves an item (the Close button already does this via `MenuButtonBase`). All five interactive items set `mouse_default_cursor_shape = CURSOR_POINTING_HAND`. `main_menu.gd`'s up/down re-focus fallback is guarded by `not settings_menu.visible` so arrow keys can't focus a button behind the overlay. Shared `settings.tscn` gives the pause-menu settings the same behavior. When the cursor leaves every item (focus null), `settings.gd` `_unhandled_input` re-grabs focus on the Master slider on `ui_up` / `ui_down`, so arrow-key navigation keeps working after focus is fully lost.
