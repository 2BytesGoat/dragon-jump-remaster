...
- **Decided to implement Tier 1 reward juice now** (see [[backlog/game_juice_plan]]): score count-up roll, floating "+N" popup on clear, rank-colored screen flash on clear, medal-bar pulse on band change, and placeholder SFX for clear/gold/death using the existing asset pool.
