...
> **Aug 31 hard deadline:** Ship a documented Godot → web → Emulation Station → Switch pipeline that jam participants can replicate. The game is the test case — it needs to be *playable enough*, not polished. See [[direction/release_plan]] § Release milestones.
...
| 50 | [x] Telemetry system | Local analytics autoload — shipped 2026-07-25 (see [[systems/architecture]]) | Medium |
 
## Phase 3 — Reproducible arcade pipeline (Aug 31 hard deadline)

> Goal: a documented Godot → web → Emulation Station → Switch pipeline that jam participants can replicate. The game only needs to be playable enough to prove the pipeline works. See [[direction/arcade_mode_design]] for the full design (implementation tracked here).
...
| # | Task | Notes | Priority |
|---|------|-------|----------|
| 35 | Decide Steam vs arcade ordering and write it in [[tracking/decisions]] | Pipeline ships first; Steam is post-pipeline | High |
...
| 51 | Anti-piracy checklist | Started 2026-08-02 — `docs/tracking/anti_piracy_checklist.md` exists but incomplete (Phase 1 done; Phases 2–5 pending) | Medium |
...
| 57 | Arcade HUD — integrate `arcade_hud.tscn` | Lives, score, level badge; draft exists at [[systems/ui_components/arcade_hud_integration_draft]] | Medium |
