...
## Overview
The Leaderboard System displays performance rankings for each level. **Online leaderboards (`LeaderboardManager` + SilentWolf) are deferred to post-launch** (see [[systems/architecture]].) In V1.0 the system is a **local UI stub only**: `leaderboard.gd` renders a "Leaderboard disabled in V1.0." placeholder, and the local arcade run summary is tracked by `ArcadeDirector`.
...
- [[systems/ui_components/ui_components.md]]
